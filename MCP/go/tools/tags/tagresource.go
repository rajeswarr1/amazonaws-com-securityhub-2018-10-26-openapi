...
func CreateTagresourceTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_tags_ResourceArn",
		mcp.WithDescription("Adds one or more tags to a resource."),
		mcp.WithString("ResourceArn", mcp.Required(), mcp.Description("The ARN of the resource to apply the tags to.")),
		mcp.WithObject("Tags", mcp.Required(), mcp.Description("Input parameter: The tags to add to the resource. You can add up to 50 tags at a time. The tag keys can be no longer than 128 characters. The tag values can be no longer than 256 characters.")),
	)
